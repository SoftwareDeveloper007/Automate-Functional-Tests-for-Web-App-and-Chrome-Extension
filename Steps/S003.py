...
class S003():
    ...
    def startSteps(self):
        ...
        ''' 11. Click on 'Stop Sharing' '''
        pTxt = "\n11. Click on 'Stop Sharing'\n"
        print(pTxt)

        try:
            stop_btn = WebDriverWait(self.driver, 50).until(
                EC.visibility_of_element_located(
                    (By.CSS_SELECTOR, "a#removeShareCollection")
                )
            )
        ...
        ''' 12. Navigate to the 'Shared Link' from the test S-001 '''
        pTxt = "\n12. Navigate to the 'Shared Link' from the test S-001\n"
        print(pTxt)
        self.driver.get(copied_link)

        try:
            error_txt = WebDriverWait(self.driver, 50).until(
                EC.presence_of_element_located(
                    (By.CSS_SELECTOR, "div.box-image-wrapper-404")
                )
            )
...
if __name__ == '__main__':
    app = S003(url='staging.getkumbu.com', email='kumbutest@mailinator.com', password='kumbu is cool',
